...
| ツール | 引数 | 内容 |
|---|---|---|
| list_presets | なし | 52プリセットと音源一覧 |
| get_preset | index: 0〜51 | 編集可能な完全パッチ |
...
既存プリセットはindex 0〜11のままです。追加40種は12〜51で、PSG・OPLL・OPN・OPNA・OPMの順に各8種を収録しています。
`list_presets`で名前と音源を確認し、`get_preset`で取得したパッチを編集してください。
最後のindex 51は「OPM / パワーアップ」です。音源ごとにコイン・決定音・ジャンプ・レーザー・爆発/衝撃・ベル・警報・パワーアップがあります。
