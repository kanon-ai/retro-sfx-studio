...
function drawGraphs(){if(!patch)return;for(const id of['pitchGraph','envelopeGraph']){const[ctx,w,h]=canvas(id);grid(ctx,w,h,'#40503e');ctx.strokeStyle='#c1d98f';ctx.lineWidth=1.4;ctx.beginPath();for(let i=0;i<=250;i++){const x=i/250,t=x*patch.duration;let y;if(id==='pitchGraph'){const semi=patch.sweep*x**patch.curve+patch.vibrato*Math.sin(t*patch.speed*Math.PI*2)+(patch.sequenceOn?patch.sequence[Math.min(7,Math.floor(x*8))]:0);y=h/2-semi/Math.max(24,Math.abs(patch.sweep)+24)*h*.42;}else y=h-5-env(t)*(h-12);if(i===0)ctx.moveTo(0,y);else ctx.lineTo(x*w,y);}ctx.stroke();ctx.fillStyle='#a8b993';ctx.font='8px monospace';ctx.fillText(id==='pitchGraph'?'PITCH / TIME':'AMPLITUDE / TIME',6,10);}}
function drawScope(buffer){const[ctx,w,h]=canvas('scope');grid(ctx,w,h,'#31482035');ctx.strokeStyle='#30461f';ctx.lineWidth=1;ctx.beginPath();if(buffer){const data=buffer.getChannelData(0);for(let x=0;x<w;x++){let lo=0,hi=0;const start=Math.floor(x*data.length/w),end=Math.floor((x+1)*data.length/w);for(let i=start;i<end;i++){lo=Math.min(lo,data[i]);hi=Math.max(hi,data[i]);}ctx.moveTo(x,h/2-lo*h*.46);ctx.lineTo(x,h/2-hi*h*.46);}}else{ctx.moveTo(0,h/2);ctx.lineTo(w,h/2);}ctx.stroke();}
